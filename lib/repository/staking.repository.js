...
export async function deleteStakingPosition(positionId) {
  try {
    // Preserve staking events for history when deleting a position.
    // Set the stakingPositionId to NULL for events referencing this position,
    // then delete the position itself.
    await db.transaction(async (tx) => {
      await tx
        .update(stakingEvents)
        .set({ stakingPositionId: null })
        .where(eq(stakingEvents.stakingPositionId, positionId));
